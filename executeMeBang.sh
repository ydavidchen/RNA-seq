...
# This script is to be executed as its entirety, rather than interactively.

manual="
Synopsis:
    fastqc-STAR-RSEM shell pipeline for analyzing all RNA-seq FASTQ files in a user-specified directory.
Usage:
...
if [[ ${datatype} == "PE" ]]; then
    echo "=== Quality control with paired-end fastqc ==="
    ...
    if [[ ${#fastq_files[@]} -ne ${#fastqc_files[@]} ]]; then
        echo "=== Some paired-end samples failed fastqc! ===" >&2
        exit 1
    fi
...
wait
    ...
    echo "=== Quality control with single-end fastqc ==="
    mkdir ${pathout}/fastqc
    for file in ${fastq_files[@]}; do fastqc $file -t ${threads} -o ${pathout}/fastqc/ & done
    wait
    fastqc_files=(`ls ${pathout}/fastqc/*fastqc.zip`)
    if [[ ${#fastq_files[@]} -ne ${#fastqc_files[@]} ]]; then
        echo "=== Some single-end samples failed fastqc!!===" >&2
        exit 1
    fi
    multiqc -o ${pathout}/qc/ ${pathout}/fastqc/
    mv ${pathout}/fastqc ${pathout}/qc/
...
## Generate table of expected counts
